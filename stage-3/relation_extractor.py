...
class RelationExtractor:
    def __init__(self, drug_labels, disease_labels, model_name="facebook/bart-large-mnli", device=-1):
    ...
    def extract_drug_disease(self, df_entities, df_sentences, window, threshold=0.6):
        ...
        for _, row in tqdm(candidates.iterrows(), total=len(candidates)):
            ...
            # 1. Determine which is drug and which is problem for the hypothesis
            if row['entity_group_1'] == 'DRUG':
                drug, disease = row['word_1'], row['word_2']
            else:
                drug, disease = row['word_2'], row['word_1']

            # 2. THE FIX: Use {{}} to provide a placeholder for the label
            # This becomes: "This text shows that [Drug] treating [Disease] is {}."
            templ = f"This text shows that {drug} treating {disease} is {{}}."
            
            # 3. Use clearer labels for a binary check
            res = self.classifier(
                text_context, 
            ...
            # 4. Extract confidence for the "true" label
